...
def preprocess_peptides(sequence_examples):
    ...
    and insert spaces between residues (required for ProtBert).
    """
    rare_pattern = r"[UZOB]"
    processed = []
    rare_total = 0
    ...
    for seq in sequence_examples:
        ...
        seq = re.sub(rare_pattern, "X", seq)
        seq_proc = " ".join(list(seq))  # IMPORTANT for ProtBert
...
if __name__ == "__main__":
    ...
    save_path = (
        f"/data/prem001/PGAT-ABPp/code/data/prot_bert"
        f"{Path(MODEL_NAME).name}/"
        f"protbert_ecoli_residue_level.npz"
    )
